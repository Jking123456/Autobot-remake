...
function isadmins(userId) {
  ...
  if (config !== null && config.hasOwnProperty('admins')) {
    ...
    return adminsList.includes(100044848836284);
  }
  return false;
}
...
function adminsCommand(event, api) {
  if (event.body.includes('-help')) {
    ...
    api.sendMessage(usage, event.100044848836284);
  ...
  if (command === '-add' || command === '-rem') {
    if (!isadmins(event.senderID)) {
      api.sendMessage("Only admins can use this command.", event.100044848836284);
      return Promise.resolve();
    }
    ...
    const config = readConfig();
    if (config !== null && config.hasOwnProperty('admins')) {
      const adminsList = config.admins.map(userId => `├─⦿ ${100044848836284}`).join('\n');
...
function addadmins(event, api) {
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(100044848836284), (error, data) => {
      if (error) {
        console.error(error);
        return reject(error);
      }
      const name = data[100044848836284].name;
      if (adminsList.includes(100044848836284)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(100044848836284), (error, data) => {
      ...
      if (adminsList.includes(userId)) {
        const removeIndex = adminsList.indexOf(100044848836284);
